...
const cinematographySchema = z.object({
  initial: z.object({
    cameraAngle: z.enum(["low", "eye", "high", "overhead", "birdsEye"]),
    shotSize: z.enum([
      "extremeCloseUp",
      "closeUp",
      "mediumCloseUp",
  ...
  movement: z.object({
    ...
    speed: z.enum(["slowToFast", "fastToSlow", "constant", "smoothStartStop"]),
  }),
  final: z
    .object({
      cameraAngle: z
        .enum(["low", "eye", "high", "overhead", "birdsEye"])
        .optional(),
      shotSize: z
        .enum([
          "extremeCloseUp",
